...
void ApplyConfig() {
    std::vector<int> B, S;
    B = string_to_mas(BirthInput.content);
    S = string_to_mas(SurviveInput.content);
    for (auto el : B)
        std::cout << el << ' ';
    std::cout << std::endl;
...
void switchNbrtype() {
    if (Gl::mode.getTypenbrs() == "4")
        Gl::mode.setTypenbrs("8");
    else if (Gl::mode.getTypenbrs() == "8")
        Gl::mode.setTypenbrs("12");
    else if (Gl::mode.getTypenbrs() == "12")
        Gl::mode.setTypenbrs("24");
    else if (Gl::mode.getTypenbrs() == "24")
        Gl::mode.setTypenbrs("4");
}
int main() {
    //ЗАГРУЗКА РЕСУРСОВ
    sf::Font myfont;
    ...
    info_typeA = Text(iw, "", &myfont, 20, sf::Color::White, {0, 0});
    ...
    Slider controlTps(iw, "TPS:", {{0, 0}, sf::Vector2f{220, 70}}, default_texture, ex_sl_t, [](int a) {Config::TPS = a;}, 1, 100, Config::TPS);
    Slider controlChance(iw, "CHANCE:", {{0, 0}, sf::Vector2f{ 220, 70 }}, default_texture, ex_sl_t, [](int a) {Config::chance = a;}, 0, 100, Config::chance);
    Slider controlGeneration(iw, "CNT GENERATION: ", { {0, 0}, sf::Vector2f(220, 70) }, default_texture, &myfont, 15, sf::Color::Black, [](int a) {Gl::mode.setCntGen(a); initGenerColors(field.cellColor);}, 2, 100, Gl::mode.getCntGen());
    Slider controlCustomGen(iw, "SIZE: ", { {0, 0}, sf::Vector2f(220, 70) }, default_texture, &myfont, 15, sf::Color::Black, [](int a) { field.sizeSquare = a; }, 1, 100, 10);
    ...
    Button newrandombut(iw, "NEW\nRANDOM", {{0, 0}, sf::Vector2f(100, 50)}, default_texture, &myfont, 13, sf::Color::Black, []() {generate_automaton(); Gl::mode = Gl::tempA;}, {1, false});
    ...
    Button typenbrs(iw, "NEIGHBOURS:", {{0, 0}, sf::Vector2f(150, 50)}, default_texture, &myfont, 14, sf::Color::Black, switchNbrtype);
    
    Button applyConfig(iw, "Apply", { {0, 0}, sf::Vector2f(100, 50) }, default_texture, ex_but_t, ApplyConfig);
    ...
    {&live, { 21, 0 }}, { &maze, {22, 0} }, { &caves, {23, 0} }, { &dayNight, { 24, 0 } }, { &amoeba, {25, 0} }, { &carpet, {26, 0} }, };
